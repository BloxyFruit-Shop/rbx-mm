...
export const createChat = mutation({
  ...
  handler: async (ctx, { tradeAd, participantIds, session }): Promise<Id<"chats">> => {
    ...
    // Check if chat already exists with these exact participants and trade ad
...
export const updateTradeStatusByMiddleman = mutation({
  ...
  handler: async (ctx, { chatId, session, newStatus }): Promise<{ success: boolean }> => {
    ...
    // Update chat trade status and clear active trade offer
    await ctx.db.patch(chatId, {
