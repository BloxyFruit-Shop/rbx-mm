...
function UserMenu() {
  ...
  return (
    <DropdownMenu>
      ...
      <DropdownMenuContent
        className="w-(--radix-dropdown-menu-trigger-width) min-w-56 rounded-xl border border-white/10 bg-gradient-to-b from-(--gag-bg)/60 to-(--gag-bg-light)/60 shadow-2xl"
        side={"bottom"}
        align="end"
        sideOffset={8}
      >
        ...
        <div className="p-1">
          ...
          >
            <div className="absolute inset-0 bg-gradient-to-r from-red-500/10 to-red-600/10 opacity-0 transition-opacity duration-200 group-hover:opacity-100" />
            <div className="relative z-10 flex items-center gap-3">
              <ArrowLeftSquare className="size-4 text-red-400 transition-colors duration-200 group-hover:text-red-300" />
...
export function UserMenuSkeleton() {
  return (
    <div className="relative flex items-center gap-2 overflow-hidden rounded-md border border-white/10 bg-gradient-to-r from-[#3A2564]/80 to-[#2A1854]/80 px-3 py-2 shadow-lg">
      ...
      <Skeleton className="ml-auto h-4 w-4 bg-white/20" />
    </div>
  );
}
