  ...
  return (
    <div className="space-y-2">
      <div className="flex items-center gap-2">
        ...
        <h4 className="text-xs font-medium sm:text-sm text-white/80">{title}</h4>
...
export function TradeOfferCard({ message, showAvatar, sessionId }: TradeOfferCardProps) {
  const [isLoading, setIsLoading] = useState(false);
  const { tradeOffer } = message;
  
  const updateTradeOfferStatus = useMutation(api.chatTradeOffers.updateTradeOfferStatus);
  ...
  const handleDecline = async () => {
    ...
    try {
      ...
      toast.error(error instanceof Error ? error.message : "Failed to decline trade offer.");
    } finally {
      setIsLoading(false);
    }
  };
            ...
            {tradeOffer.status === "pending" && !message.isCurrentUser && (
              <div className="flex gap-2 pt-2">
                <Button
                  ...
                  disabled={isLoading}
                  className="flex-1 text-white bg-gradient-to-r from-green-500 to-green-600 hover:from-green-600 hover:to-green-700 disabled:opacity-50"
                >
