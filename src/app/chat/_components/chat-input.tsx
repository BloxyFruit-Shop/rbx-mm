...
export function ChatInput({ 
  ...
  // Determine if chat is active (not completed or cancelled)
  const isChatActive = tradeStatus !== 'completed' && tradeStatus !== 'cancelled';
  
  // Determine if new trade offers can be created (only when no active trade and user is not the trade ad creator)
  const canCreateTradeOffer = tradeStatus === 'none' && !isMiddleman && tradeAdCreatorId !== currentUserId;
  
  // Determine if middleman call can be made (only when trade is accepted and user is not middleman)
  const canCallMiddleman = tradeStatus === 'accepted' && !isMiddleman && isChatActive;
