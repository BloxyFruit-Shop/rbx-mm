...
export default function MiddlemanClient() {
  ...
  // Fetch middleman calls
  const middlemanCalls = useQuery(
  ...
  // Filter and sort middleman calls
  const filteredAndSortedCalls = useMemo(() => {
    if (!middlemanCalls) return [];

    let filtered = middlemanCalls;
    ...
    return filtered;
  }, [middlemanCalls, debouncedSearchTerm]);
          ...
          <div className={`${sidebarOpen ? "flex-1" : "w-full"} overflow-hidden`}>
            <div className="mb-4 sm:mb-6">
              ...
              <p className="text-xs sm:text-sm text-white/70">
                {t('showingResults', { filtered: filteredAndSortedCalls.length, total: stats?.total ?? 0 })}
              </p>
            </div>
