  ...
  return (
    <div className="flex gap-2">
      {displayItems.map((item, index) => (
        <div
          key={`${item._id}-${index}`}
          className="relative overflow-hidden border rounded size-12 border-white/10 bg-gradient-to-br from-white/5 to-white/10"
        >
          <Image
      ...
      {remainingCount > 0 && (
        <div className="flex items-center justify-center border border-dashed rounded size-12 border-white/20 bg-white/5">
...
export default function TradeAdCard({
  ...
  return (
    <>
      <Card
        className={cn(
          "group relative overflow-hidden bg-gradient-to-br from-white/5 to-white/10 backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:border-white/30 hover:shadow-xl hover:shadow-blue-500/20 flex flex-col h-full",
            ...
            <div className="flex items-center flex-1 min-w-0 gap-3">
              <div className="relative">
                <Avatar className="size-12 ring-2 ring-white/20">
                  ...
                  />
                  <AvatarFallback className="font-semibold text-white bg-gradient-to-br from-purple-500 to-blue-500">
                    {tradeAd.creator?.name?.charAt(0) ?? "?"}
                  </AvatarFallback>
                </Avatar>
                <div className="absolute p-1 rounded-full -right-1 -bottom-1 bg-white/10 backdrop-blur-sm">
                  <StatusIcon className="text-white size-3" />
                </div>
              </div>
        ...
        <CardContent className="flex-1 space-y-4">
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <Package className="text-green-400 size-4" />
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-medium text-white/80">
                Wanting {isHearingOffers ? "(Any Offers)" : `(${tradeAd.wantItemsResolved.length})`}
              </span>
            </div>
            {isHearingOffers ? (
              <div className="flex items-center justify-center p-4 border border-dashed rounded-lg border-green-500/30 bg-gradient-to-br from-green-500/10 to-emerald-500/10">
                <div className="text-center">
                  <div className="flex items-center justify-center w-8 h-8 mx-auto mb-2 rounded-full bg-green-500/20">
                    <Sparkles className="text-green-400 size-4" />
                  </div>
          ...
          {tradeAd.notes && (
            <div className="p-3 border rounded-lg border-white/10 bg-gradient-to-br from-white/5 to-white/10">
              <p className="text-sm text-white/80 line-clamp-2">{tradeAd.notes}</p>
            </div>
          )}
        </CardContent>

        <CardFooter className="pt-2 mt-auto @container">
          <div className="flex flex-col w-full gap-2 @sm:flex-row">
            <Button
              variant="outline"
              size="sm"
              onClick={onSeeDetails}
              className="flex-1 py-2 border-white/20 bg-white/5 hover:border-white/30 hover:bg-white/10"
            >
            ...
            {tradeAd.status === "open" && (
              <div className="flex gap-2">
                ...
                <Button
                  ...
                  disabled={isCreatingChat}
                  className="flex-1 text-white border-0 shadow-lg bg-gradient-to-r from-blue-500 to-purple-600 shadow-blue-500/25 hover:from-blue-600 hover:to-purple-700 disabled:opacity-50 @sm:flex-none"
                >
